...
export const getRefinedSearchRequest = (filter, token) => {
  let headers = {
  ...
  return axios.get(`/api/refined_search?filter=${filter}`, { headers });
};
...
export default {
  ...
  effects: (dispatch) => ({
    ...
    async fetchRefinedSearch(filter, rootState) {
      try {
        const token = rootState.searchModel.token;
        const response = await getRefinedSearchRequest(filter, token);
